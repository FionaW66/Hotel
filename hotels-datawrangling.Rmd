...
My intuition tells me that it's more likely to find bookings with children or babies in city hotels.
Resort: 3929
City: 5403
City hotels have more children or babies.
...
The most common number of adults in booking is 2. 403 bookings have 0 adults, which is surprising. There are a couple 20 +, which is weird too. 
...
For the canceled bookings, we can see that those really high numbers of adults all canceled. For 0 adults, some canceled, but majority didn't, indicating that some hotels are letting children book by themselves.

```{r canceled adult frequency}
hotels %>% 
  filter(is_canceled == 1) %>% 
  count(adults, sort = TRUE)
```
```{r not canceled adult frequency}
hotels %>% 
...
City hotel is higher.

```{r two hotels descriptives}
hotels %>% 
  group_by(hotel) %>% 
  summarize(min(adr, na.rm=TRUE), max(adr, na.rm=TRUE), 
            mean(adr, na.rm=TRUE), median(adr, na.rm = TRUE))
```
